...
public class Util {
	...
	public static void initTextureData() throws IOException {
		...
		if (!logoFile.exists()) {
			InputStream defaultLogoInputStream = Minecraft.getInstance().getResourceManager().open(Identifier.fromNamespaceAndPath("minecraft", "textures/gui/title/minecraft.png"));
		...
		if (ConfigHandler.replaceMainMenuLogo) {
			Identifier logoIdentifier = Identifier.fromNamespaceAndPath(Reference.MOD_ID, "logo");

			InputStream logoInputStream = new FileInputStream(logoFile);
			NativeImage logoNativeImage = NativeImage.read(logoInputStream);
			DynamicTexture logoTexture = new DynamicTexture(logoIdentifier::toString, logoNativeImage);

			Minecraft.getInstance().getTextureManager().register(logoIdentifier, logoTexture);
		...
		if (!logoEditionFile.exists()) {
			InputStream defaultEditionInputStream = Minecraft.getInstance().getResourceManager().open(Identifier.fromNamespaceAndPath("minecraft", "textures/gui/title/edition.png"));
		...
		if (ConfigHandler.replaceMainMenuEditionLogo) {
			Identifier editionIdentifier = Identifier.fromNamespaceAndPath(Reference.MOD_ID, "edition");
			...
			DynamicTexture logoEditionTexture = new DynamicTexture(editionIdentifier::toString, logoEditionNativeImage);

			Minecraft.getInstance().getTextureManager().register(Identifier.fromNamespaceAndPath(Reference.MOD_ID, "edition"), logoEditionTexture);
		...
		if (ConfigHandler.setCustomBackground) {
			Identifier backgroundIdentifier = Identifier.fromNamespaceAndPath(Reference.MOD_ID, "background");
			...
			DynamicTexture backgroundTexture = new DynamicTexture(backgroundIdentifier::toString, backgroundNativeImage);

			Minecraft.getInstance().getTextureManager().register(backgroundIdentifier, backgroundTexture);
